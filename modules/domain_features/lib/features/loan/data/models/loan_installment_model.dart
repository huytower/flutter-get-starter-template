import 'package:app_config/data/datasource/local/box/cc_hive_box.dart';
import 'package:hive_ce/hive_ce.dart';

import '../../domain/entities/loan_entity.dart';

part 'loan_installment_model.g.dart';

/// Embedded (not its own box) — always read/written as part of a
/// [LoanModel]'s `installments` list, mirrors
/// `ReconciliationAllocationModel`.
@HiveType(typeId: CcHiveBox.LOAN_INSTALLMENT_TYPE_ID)
class LoanInstallmentModel {
  @HiveField(0)
  final String dueDate;

  @HiveField(1)
  final int amount;

  LoanInstallmentModel({required this.dueDate, required this.amount});

  factory LoanInstallmentModel.fromEntity(LoanInstallmentEntity entity) =>
      LoanInstallmentModel(
        dueDate: entity.dueDate.toIso8601String(),
        amount: entity.amount,
      );

  LoanInstallmentEntity toEntity() =>
      LoanInstallmentEntity(dueDate: DateTime.parse(dueDate), amount: amount);

  Map<String, dynamic> toJson() {
    return {'dueDate': dueDate, 'amount': amount};
  }

  factory LoanInstallmentModel.fromJson(Map<String, dynamic> json) {
    return LoanInstallmentModel(
      dueDate: json['dueDate'] as String,
      amount: json['amount'] as int,
    );
  }
}
