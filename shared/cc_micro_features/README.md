...
A collection of **project-blind, reusable Micro-Features** following the Hybrid-Modular Super App architecture. These
are standalone business verticals (Feature-as-a-Service) designed to be shared across multiple enterprise applications.

## 🎯 The "Global Lego" Principle

Micro-Features are engineered as independent blocks. They follow two strict rules:

1. **Project-Blind**: They must NEVER depend on the App Shell (`lib/`) or app-specific data modules. They only
   communicate via interfaces (Dependency Inversion).
2. **State-Management Agnostic Core**: While they may use Bloc or GetX in their `presentation/` layer, their business
   logic (domain/data) is strictly decoupled and agnostic.
...
If a Micro-Feature needs to access project-specific data (e.g., an app-specific Auth implementation), it MUST NOT import
it. Instead:
