...
1. **Hybrid-Modular Super App Design (CRITICAL)**: The project is divided into three distinct layers:
    ...
    - **Micro-Features (Global Legos)**: The `cc_micro_features/` folder. Project-blind business verticals (Auth,
      Biometric, etc.) reusable across different enterprise apps.
...
3. **State-Management Agnostic Core (STRICT)**:
    - `cc_core_sdk` modules (`cc_sdk`, `cc_sdk_ui`, `cc_mixin`, `cc_bridge`) MUST NOT depend on specific state
      management (GetX/Bloc).
    - Features may use state management (Bloc/GetX) in their `presentation/` layer, but their `domain/` and `data/`
      layers must remain agnostic.
